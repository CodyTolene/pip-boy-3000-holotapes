(function() {
    ...
    function gameStart() {
        ...
        let startingHP = gameType === 'commander' ? C.GAME_COMMANDER_HP : C.GAME_STANDARD_HP;
        ...
        for (let i = 0; i < playerCount; i++) {
            ...
            for (let j = 0; j < (playerCount === 1 ? 6 : playerCount); j++) {
                player.commanderDamageSources.push(new CommanderDamageSource(j, 'Player ' + (j + 1)));
            }
    ...
    function handleLeftKnob(dir, long) {
        ...
        if (dir !== 0 && inGame === false) {
            ...
            } else if (gameType === 'commander' && menuDisplayed === 'commanderDamage') {
                Pip.playSound("SCROLL");
                menuDisplayed = '';
                drawGameBoard();
            } else if (menuDisplayed === 'inGameOptions') {
    ...
    function handleRightKnob(dir) {
        ...
        if (inGame === false) {
            if (menuDisplayed === 'gameNew' && menuIndexSelected === 0) {
                ...
                playerCount = limitNumberWithinRange(playerCount + dir, C.PLAYER_COUNT_MIN, C.PLAYER_COUNT_MAX);
                drawMenu(MENU_GAME_NEW_OPTIONS);
                if (prevIndex !== playerCount) {
                    Pip.playSound("SCROLL");
                }
            }
        } else if (inGame === true && menuDisplayed !== 'commanderDamage' && menuDisplayed !== 'inGameOptions') {
