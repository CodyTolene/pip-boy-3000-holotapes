(function () {
  ...
  function isPlayer(v) { return v === 2 || v === 4; }
  function isAI(v) { return v === 1 || v === 3; }
  ...
  function getJumps(r, c, isPlayerPiece) {
    ...
    for (let i = 0; i < dirs.length; i++) {
      ...
      if (midIsEnemy && isEmpty(target)) {
        jumps.push({from:{row:r,col:c}, to:{row:tr,col:tc}, capture:{row:mr,col:mc}});
      }
    }
    return jumps;
  }
  ...
  function getSimpleMoves(r, c, isPlayerPiece) {
    ...
    for (let i = 0; i < dirs.length; i++) {
      ...
      if (isEmpty(board[tr][tc])) {
        moves.push({from:{row:r,col:c}, to:{row:tr,col:tc}, capture:null});
      }
    }
    return moves;
  }
  ...
  function getMovesForPiece(r, c, isPlayerTurn) {
    ...
    for (let i = 0; i < validMoves.length; i++) {
      if (validMoves[i].capture) { hasJumps = true; break; }
    }
    return hasJumps ? getJumps(r, c, isPlayerTurn) : getSimpleMoves(r, c, isPlayerTurn);
  }
  ...
  function endAITurn() {
    let w = checkWin();
    if (w) { winner = w; gameState = "over"; }
    else {
      gameState = "player";
      validMoves = getValidMoves(true);
      selected = null; selectedMoves = []; mustContinue = null;
    }
    draw();
  }
  ...
  function aiMove() {
    ...
    if (moves.length === 0) { winner = "player"; gameState = "over"; draw(); return; }
    let jumps = moves.filter(function(m) { return m.capture !== null; });
    let candidates = jumps.length > 0 ? jumps : moves;
    let kingMoves = candidates.filter(function(m) { return m.to.row === 7; });
  ...
  function onPress() {
    if (gameState === "over") { initBoard(); draw(); return; }
    if (gameState !== "player") return;
    let r = curRow, c = curCol;
    if (mustContinue && (r !== mustContinue.row || c !== mustContinue.col)) return;

    if (selected) {
      let dest = null;
      for (let i = 0; i < selectedMoves.length; i++) {
        let m = selectedMoves[i];
        if (m.to.row === r && m.to.col === c) { dest = m; break; }
      }
      if (dest) {
        executeMove(dest);
        if (dest.capture) {
          ...
          if (more.length > 0) {
            ...
            selectedMoves = more;
            curRow = dest.to.row; curCol = dest.to.col;
            draw(); return;
          }
        }
        let w = checkWin();
      ...
      if (r === selected.row && c === selected.col) {
        selected = null; selectedMoves = []; draw(); return;
      }
    }

    if (isPlayer(board[r][c])) {
      let moves = getMovesForPiece(r, c, true);
      if (moves.length > 0) { selected = {row:r,col:c}; selectedMoves = moves; }
    }
    draw();
  }

  function cellSX(c) { return GRID_X + c * CELL; }
  function cellSY(r) { return GRID_Y + r * CELL; }

  function isValidDest(r, c) {
    for (let i = 0; i < selectedMoves.length; i++) {
      if (selectedMoves[i].to.row === r && selectedMoves[i].to.col === c) return true;
    }
    return false;
  }
  ...
  function draw() {
    ...
    if (gameState === "ai") {
      ...
      h.setColor(winner === "player" ? 3 : 1).setFontMonofonto16().setFontAlign(1, 0)
    ...
    for (let r = 0; r < 8; r++) {
      for (let c = 0; c < 8; c++) {
        ...
        if (isCursor) {
          h.setColor(3).drawRect(sx+1, sy+1, sx+CELL-1, sy+CELL-1)
        ...
        if (piece > 0) {
          ...
          if (isPlayer(piece)) {
            ...
            if (isKing(piece)) {
              h.setColor(1).setFontMonofonto16().setFontAlign(0, 0)
    ...
    // Bottom bar
    h.setColor(2).setFontMonofonto16().setFontAlign(0, 0)
      .drawString(gameState === "over" ? "PRESS TO PLAY AGAIN" : "SELECT + CONFIRM TO MOVE", 240, 308);

    h.flip();
    Pip.lastFlip = getTime();
  }

  function onKnob1(d) { if (gameState !== "player") return; moveCursor(d, 0); draw(); }
  function onKnob2(d) { if (gameState !== "player") return; moveCursor(0, d); draw(); }
